...
梯度消失与梯度爆炸其实是一种情况，看接下来的文章就知道了。两种情况下梯度消失经常出现，一是在**深层网络**中，二是采用了**不合适的损失函数**，比如[sigmoid](../基础理论/神经网络组件/激活函数/sigmoid.md)。梯度爆炸一般出现在深层网络和**权值初始化值太大**的情况下，下面分别从这两个角度分析梯度消失和爆炸的原因。
...
其实也注意到了，上文中提到计算权值更新信息的时候需要计算前层偏导信息，因此如果激活函数选择不合适，比如使用[sigmoid](../基础理论/神经网络组件/激活函数/sigmoid.md)，梯度消失就会很明显了，原因看下图，左图是[sigmoid](../基础理论/神经网络组件/激活函数/sigmoid.md)的损失函数图，右边是其倒数的图像，如果使用[sigmoid](../基础理论/神经网络组件/激活函数/sigmoid.md)作为损失函数，其梯度是不可能超过0.25的，这样经过链式求导之后，很容易发生梯度消失，sigmoid函数数学表达式为： [sigmoid](../基础理论/神经网络组件/激活函数/sigmoid.md)
...
同理，tanh作为损失函数，它的导数图如下，可以看出，tanh比[sigmoid](../基础理论/神经网络组件/激活函数/sigmoid.md)要好一些，但是它的倒数仍然是小于1的。tanh数学表达为：
